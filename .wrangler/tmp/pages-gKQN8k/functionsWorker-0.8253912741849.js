...
// api/vecinity-pay/create-order.js
async function onRequestPost(context) {
  const { request, env } = context;
  const url = new URL(request.url);
  const path2 = url.pathname;
  const currentFlowUrl = env.FLOW_MODE === "live" ? FLOW_API_URL : FLOW_SANDBOX_URL;
  const headers = {
    "Content-Type": "application/json",
    "Access-Control-Allow-Origin": "https://vecinoslaserena.cl"
  };
  try {
    if (path2.endsWith("/create-order")) {
      const body = await request.json();
      const { itemId, engine, userId, email, amountCLP, amountUSD, type, fichas } = body;
      if (!itemId || !engine || !userId || !email) {
        return new Response(JSON.stringify({ error: "Par\xE1metros incompletos" }), { status: 400, headers });
      }
      ...
      if (engine === "national") {
        const flowParams = new URLSearchParams({
          ...
          subject: `VLS: ${type === "fichas" ? fichas + " Fichas VLS" : "Suscripci\xF3n Pro " + itemId}`,
        ...
        const signatureBase = [...flowParams.entries()].sort(([a], [b]) => a.localeCompare(b)).map(([k, v]) => k + v).join("");
        ...
        const ppOrderRes = await fetch(`${ppBase}/v2/checkout/orders`, {
          ...
          body: JSON.stringify({
            ...
            purchase_units: [{
              ...
              description: type === "fichas" ? `${fichas} Fichas VLS` : `Suscripci\xF3n Pro - ${itemId}`,
              amount: { currency_code: "USD", value: amountUSD.toFixed(2) },
              custom_id: JSON.stringify({ userId, fichas, type, itemId })
            }],
            application_context: {
              brand_name: "Vecinity \xB7 VecinosLaSerena",
              locale: "es-CL",
...
var FLOW_API_URL, FLOW_SANDBOX_URL;
var init_create_order = __esm({
  "api/vecinity-pay/create-order.js"() {
    init_functionsRoutes_0_346422734067677();
...
async function onRequestPost_flow(context) {
  const { request, env } = context;
  ...
  try {
    ...
    const signatureBase = [...params.entries()].sort(([a], [b]) => a.localeCompare(b)).map(([k, v]) => k + v).join("");
    const encoder = new TextEncoder();
    const keyData = encoder.encode(env.FLOW_SECRET_KEY);
    const msgData = encoder.encode(signatureBase);
    const cryptoKey = await crypto.subtle.importKey("raw", keyData, { name: "HMAC", hash: "SHA-256" }, false, ["sign"]);
    ...
    const apiBase = env.FLOW_MODE === "live" ? "https://www.flow.cl/api" : "https://sandbox.flow.cl/api";
    ...
    if (paymentData.status === 2) {
      ...
      await sendP2PNotification(
        userId,
        `\u2705 \xA1Fichas cargadas! Tu pago de $${paymentData.amount.toLocaleString("es-CL")} CLP fue procesado con \xE9xito. Tienes ${newBalance} Fichas VLS disponibles. \xA1Disfruta el ecosistema! \u{1F39F}\uFE0F`,
        env
      );
...
async function onRequestPost_paypal(context) {
  const { request, env } = context;
  ...
  try {
    ...
    if (verification.verification_status !== "SUCCESS") {
      console.warn("[WebhookPayPal] Firma inv\xE1lida:", verification);
    ...
    await sendP2PNotification(
      userId,
      `\u2705 \xA1Pago Internacional Aprobado! USD ${amountUSD} procesados v\xEDa PayPal. ${fichas} Fichas VLS acreditadas. Saldo actual: ${newBalance} \u{1F39F}\uFE0F`,
      env
    );
...
var init_webhook = __esm({
  "api/vecinity-pay/webhook.js"() {
    ...
    __name(updateFirebaseTokens, "updateFirebaseTokens");
    __name(sendP2PNotification, "sendP2PNotification");
    __name(onRequestPost_flow, "onRequestPost_flow");
    __name(onRequestPost_paypal, "onRequestPost_paypal");
...
async function onRequestPost3({ request, env }) {
  ...
  try {
    ...
    const chat = model.startChat({
      history: history || [],
      systemInstruction: "Eres 'Farito', el asistente inteligente de vecinoslaserena.cl y farito.cl. Tu rol es ser un enrutador inteligente. Si el usuario reporta un problema, sugiere los canales oficiales de la plataforma (Radar Vecinal, Reportes). Mant\xE9n un tono servicial, r\xE1pido y muy local de La Serena, Chile."
    });
...
var init_chat = __esm({
  "api/chat.js"() {
    init_functionsRoutes_0_346422734067677();
    ({ GoogleGenAI: GoogleGenAI2 } = (init_web(), __toCommonJS(web_exports)));
...
async function onRequestPost4(context) {
  ...
  try {
    ...
    Determina toxicidad (isToxic), sentimiento (alegr\xEDa, enojo, preocupaci\xF3n, neutro) y tema principal.
...
var GoogleGenAI3;
var init_moderate = __esm({
  "api/moderate.js"() {
    init_functionsRoutes_0_346422734067677();
    ({ GoogleGenAI: GoogleGenAI3 } = (init_web(), __toCommonJS(web_exports)));
    __name(onRequestPost4, "onRequestPost");
  }
});

// api/airport-monitoring.js
async function onRequest(context) {
  const today = /* @__PURE__ */ new Date();
  const dayStr = today.toLocaleDateString("es-CL");
  const hour = today.getHours();
  const getStatus = /* @__PURE__ */ __name((flightHour) => {
  ...
  const arrivals = [
    ...
    { airline: "JETSMART", flight: "JA 205", origin: "CONCEPCI\xD3N", date: dayStr, time: "14:15", belt: "1", status: getStatus(14) },
...
var init_airport_monitoring = __esm({
  "api/airport-monitoring.js"() {
    init_functionsRoutes_0_346422734067677();
...
async function onRequest2(context) {
  ...
  const totalServed = (2 + Math.random() * 0.1).toFixed(2);
  const growth = "+24%";
  return new Response(JSON.stringify({
    liveUsers,
    totalServed,
...
var init_cloudflare_stats = __esm({
  "api/cloudflare-stats.js"() {
    init_functionsRoutes_0_346422734067677();
...
var init_port_monitoring = __esm({
  "api/port-monitoring.js"() {
    init_functionsRoutes_0_346422734067677();
...
async function onRequest4(context) {
  ...
  const glossary = [
    ...
    { term: "Magnitud", definition: "Medida del tama\xF1o de un sismo y de la energ\xEDa liberada cerca de la fuente." },
    { term: "Intensidad (Mercalli)", definition: "Medida del efecto de un sismo en personas, estructuras y la superficie terrestre." },
    { term: "Tsunami", definition: "Serie de olas de gran energ\xEDa generadas por desplazamientos verticales de la columna de agua." },
    { term: "Falla Geol\xF3gica", definition: "Fractura en la corteza terrestre a lo largo de la cual ha habido desplazamiento." },
    { term: "Brecha S\xEDsmica (Gap)", definition: "Zona que no ha experimentado un gran terremoto en mucho tiempo." }
  ];
  const recentQuakes = [
    { id: "vls-s1", time: (/* @__PURE__ */ new Date()).toISOString(), place: "30 km al SO de Coquimbo", mag: 3.4, depth: 45, lat: -30.12, lng: -71.45 },
    { id: "vls-s2", time: new Date(Date.now() - 36e5).toISOString(), place: "12 km al E de Vicu\xF1a", mag: 2.8, depth: 102, lat: -30.01, lng: -70.65 },
    { id: "vls-s3", time: new Date(Date.now() - 72e5).toISOString(), place: "55 km al O de Tongoy", mag: 4.1, depth: 32, lat: -30.25, lng: -71.95 },
    { id: "vls-s4", time: new Date(Date.now() - 144e5).toISOString(), place: "40 km al N de Los Vilos", mag: 3.2, depth: 28, lat: -31.62, lng: -71.55 }
  ];
  return new Response(JSON.stringify({
    success: true,
    ...
    quakes: recentQuakes,
    glossary,
    disclaimer: "Informaci\xF3n para fines de orientaci\xF3n. Siempre consulte canales oficiales."
  }), {
    headers: { "Content-Type": "application/json" }
  });
}
var init_sismologia = __esm({
  "api/sismologia.js"() {
    init_functionsRoutes_0_346422734067677();
...
// ../.wrangler/tmp/pages-gKQN8k/functionsRoutes-0.346422734067677.mjs
var routes;
var init_functionsRoutes_0_346422734067677 = __esm({
  "../.wrangler/tmp/pages-gKQN8k/functionsRoutes-0.346422734067677.mjs"() {
    init_create_order();
    init_create_order();
    init_webhook();
    init_chat();
    init_moderate();
    init_airport_monitoring();
    init_cloudflare_stats();
    init_port_monitoring();
    init_sismologia();
    routes = [
      {
