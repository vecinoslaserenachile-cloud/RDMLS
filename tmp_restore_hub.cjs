...
// Extraemos la parte "sana" (desde la línea 10 del archivo dañado)
// En el archivo dañado, la línea 10 empieza con "    if (curTenant === 'gore-coquimbo') {"
const healthyPart = lines.slice(9).join('\n');
...
export default function HubDashboard() {
    ...
    const { lang, setLang, t: contextT } = useTranslation();
    ...
    const t = { ...contextT, ...(dict[lang] || dict.es) };
...
fs.writeFileSync(filePath, header + healthyPart, 'utf8');
console.log('✅ HubDashboard.jsx RESTORED (hopefully).');
