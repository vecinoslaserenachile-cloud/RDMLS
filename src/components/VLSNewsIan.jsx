...
const CHAPTERS = [
  {
    ...
    audio: '/DonWilson/Trampas_electricas_en_el_supermercado_Santa_Isabel_-_1_.mp3',
  ...
  },
  {
    ...
    technicalDetails: [
      ...
      { id: 'Fallo 3', title: 'Omisión de Mantenimiento', desc: 'Existencia de alertas previas de descargas eléctricas no gestionadas. Aislamiento reactivo deficiente.' }
    ],
    icon: Activity,
    color: '#ef4444'
  },
  ...
  },
  {
    ...
    action: 'Lucatón: Campaña solidaria para ayudar a la familia con gastos de insumos médicos y cuidados clínicos.',
    icon: Building2,
    color: '#f59e0b'
  },
  {
...
export default function VLSNewsIan({ onClose }) {
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, background: '#020617', zIndex: 9999999, display: 'flex', flexDirection: 'column', color: 'white', fontFamily: '"Inter", sans-serif' }}>
      ...
      {/* Header Premium */}
      <header style={{ padding: '1rem 2rem', background: 'rgba(15, 23, 42, 0.9)', backdropFilter: 'blur(10px)', borderBottom: '1px solid rgba(255, 255, 255, 0.1)', display: 'flex', justifyContent: 'space-between', alignItems: 'center', zIndex: 1500000 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
          <div style={{ background: '#ef4444', color: 'white', padding: '4px 10px', fontWeight: '900', borderRadius: '4px', fontSize: '0.9rem' }}>VLS EXPERT</div>
          <div style={{ fontSize: '0.8rem', fontWeight: '700', color: 'rgba(255, 255, 255, 0.6)', textTransform: 'uppercase', letterSpacing: '1px' }}>Unidad de Investigación Legal</div>
        </div>
        <div style={{ display: 'flex', gap: '1rem' }}>
          <button onClick={handleShare} style={{ background: 'rgba(255, 255, 255, 0.1)', border: 'none', color: 'white', padding: '0.6rem', borderRadius: '50%', cursor: 'pointer' }}><Share2 size={18} /></button>
          <button onClick={onClose} style={{ background: '#ef4444', border: 'none', color: 'white', padding: '0.6rem 1.5rem', borderRadius: '50px', fontWeight: '800', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '8px' }}>
            SALIR <X size={20} />
          </button>
        </div>
      </header>
      ...
      <div id="reportage-scroll" style={{ flex: 1, overflowY: 'auto', scrollBehavior: 'smooth' }}>
        ...
        <section style={{ minHeight: '90vh', position: 'relative', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '4rem 2rem', textAlign: 'center', overflow: 'hidden' }}>
          ...
          >
            ...
            <div style={{ padding: '2rem', background: 'rgba(255, 255, 255, 0.03)', border: '1px solid rgba(255, 255, 255, 0.05)', borderRadius: '24px', display: 'flex', alignItems: 'center', gap: '1.5rem', textAlign: 'left' }}>
              <div style={{ background: '#ef4444', padding: '15px', borderRadius: '16px' }}><ShieldAlert size={32} /></div>
              <div>
        ...
        <section style={{ maxWidth: '850px', margin: '0 auto', padding: '4rem 1.5rem' }}>
          ...
          <div style={{ background: 'rgba(255, 255, 255, 0.03)', border: '1px solid rgba(255, 255, 255, 0.1)', borderRadius: '32px', padding: '3rem', textAlign: 'center' }}>
            ...
            <p style={{ fontSize: '1.1rem', lineHeight: '1.8', color: 'rgba(255, 255, 255, 0.7)', marginBottom: '2.5rem' }}>
              Ian sobrevivió de milagro a un sistema que delega la seguridad en manuales que nadie lee y que ahorra costos a costa de la integridad pública. Mientras la PDI recaba evidencias y los abogados redactan las querellas, una familia en nuestra ciudad sigue sufriendo el abismo de esta negligencia.
            </p>
            <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem', alignItems: 'center' }}>
              <p style={{ fontWeight: '800', color: '#f59e0b' }}>Hacemos un ferviente llamado a los vecinos de La Serena a seguir apoyando a la familia a través de la LUCATÓN.</p>
              <motion.button 
                whileHover={{ scale: 1.05 }}
                whileTap={{ scale: 0.95 }}
                style={{ background: 'linear-gradient(135deg, #ef4444, #991b1b)', border: 'none', color: 'white', padding: '1rem 3rem', borderRadius: '50px', fontSize: '1.2rem', fontWeight: '900', cursor: 'pointer', boxShadow: '0 20px 40px rgba(239, 68, 68, 0.3)' }}
              >
                UNIRSE A LA LUCATÓN 
              </motion.button>
            </div>
        ...
        <section style={{ background: '#0f172a', padding: '4rem 0' }}>
            <CommentSection themeColor="#ef4444" reportTitle="El Punto Ciego del Retail - VLS" />
        </section>
