...
export default function MarketplaceVecinal({ onClose }) {
    ...
    const loadListings = () => {
        ...
        if (saved.length === 0) {
            ...
            // Clean up any old dummy data if they somehow returned (e.g. from an old cache sync)
            const cleanUpdated = updated.filter(item => !['1', '2', '3'].includes(item.id));
            
            setListings(cleanUpdated);
            localStorage.setItem('vls_marketplace_listings_v2', JSON.stringify(cleanUpdated));
        }
    };
