  ...
  const renderContent = () => {
    switch (step) {
          ...
          <div className="w-full h-full bg-black rounded-[3rem] overflow-hidden shadow-2xl border border-white/20 relative">
            {!showVideo ? (
               <div className="w-full h-full relative group cursor-pointer" onClick={() => setShowVideo(true)}>
                  ...
                  <div className="absolute top-10 left-10 text-white font-black uppercase text-xl z-20">Alcaldesa Daniela Norambuena</div>
      ...
      case 8: return <ChapterLayout title="Bienestar" subtitle="Calidad de Vida" 
        visual={<div className="p-12 text-center text-xs space-y-4 text-slate-500 uppercase font-black tracking-widest leading-relaxed"><MapPin size={100} className="text-red-600 mx-auto mb-4 drop-shadow-2xl"/><div>📍 Prat 451 (Consistorial)</div><div>📍 Los Carrera 301</div><div>📍 Matta 240 (Bienestar)</div></div>}
        content={<><p className="text-3xl font-black text-white mb-8 uppercase italic">Calidad de Vida & Soporte:</p><div className="space-y-6"><div className="p-8 border-2 border-green-500/30 bg-green-500/10 rounded-[2rem] flex gap-8 items-center backdrop-blur-sm transition-all hover:bg-green-500/20"><Stethoscope size={50} className="text-green-500 shrink-0"/><div className="space-y-1"><h4 className="font-black text-white text-2xl uppercase">Apoyo Integral</h4><p className="text-slate-400 text-lg">Servicios de psicología, medicina general y beneficios maternales para todos los trabajadores.</p></div></div><div className="p-8 border-2 border-blue-500/30 bg-blue-500/10 rounded-[2rem] flex gap-8 items-center backdrop-blur-sm transition-all hover:bg-blue-500/20"><Building2 size={50} className="text-blue-500 shrink-0"/><div className="space-y-1"><h4 className="font-black text-white text-2xl uppercase">Marco Normativo</h4><p className="text-slate-400 text-lg">Es vital conocer las leyes 18.575, 18.695 y el Estatuto Administrativo 18.883.</p></div></div></div></>} 
      />;
