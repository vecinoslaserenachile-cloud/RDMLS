...
// 1. Extraer todas las importaciones
const importRegex = /^import\s+.*?;/gm;
const imports = content.match(importRegex) || [];
content = content.replace(importRegex, '');

// 2. Limpiar duplicados y basura de importaciones parciales
const uniqueImports = [...new Set(imports.map(i => i.trim()))]
    .filter(i => i.includes('from') && !i.includes('import Rea//'));

// 3. Eliminar cabeceras de funciones duplicadas
// Buscamos "export default function HubDashboard() {"
const funcHeader = "export default function HubDashboard() {";
const headerCount = (content.match(new RegExp(funcHeader, 'g')) || []).length;
if (headerCount > 1) {
    // Mantener solo la primera ocurrencia (o reconstruir)
    content = content.replace(new RegExp(funcHeader, 'g'), '');
}

// 4. Reconstrucción Final
const finalHeader = \`import React, { useState, useEffect, useRef } from 'react';
...
fs.writeFileSync(filePath, finalHeader + content, 'utf8');
console.log('🚀 HubDashboard.jsx SANITIZED & RECONSTRUCTED.');
